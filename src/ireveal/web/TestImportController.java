...
public class TestImportController extends SimpleFormController{
	...
	protected HashMap<String, Object> referenceData(HttpServletRequest request) throws Exception {
		...
		if(testtype.equals("PD") && atype.equals("C")){
			hfreq=mastersservice.getFreqdatafile("PDH",testid);
			vfreq=mastersservice.getFreqdatafile("PDV",testid);}
		else {
			hfreq=mastersservice.getFreqdatafile("H",testid);
			vfreq=mastersservice.getFreqdatafile("V",testid);
			ffreq=mastersservice.getFreqdatafile("F",testid);
			}
