...
END$$
DELIMITER;
-- new product prcedures
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `pv_Calculate_params`(
myTestId INT,
-- myFreqUnit char(1), -- 'M' = MHz, 'G' = GHz
myPoltype char(2) -- 'L'=linear, 'C'= circular
)
...
#open cursor
 ...
 FETCH freqcur INTO myfreq; -- ,mylinear_Gain;
IF done = 1 THEN
	if isDebug > 0 then
		SET @infoText = "Done looping through spot frequencies";
		call debug(l_proc_id,@infoText,'I','I');
...
-- Gain Measurement
  IF ( myPolType = 'PV' and myTestType = 'GM') THEN
	if isDebug > 0 then
		SET @infoText = CONCAT("invoking Gain-Measurement calculations for frequency : ",myfreq);
		call debug(l_proc_id,@infoText,'I','I');
	end if;
	call pv_calc_Gain_Measurement(myTestId,myfreq,myTestDate);
-- Gain TRacking
  elseif ( myPolType = 'PV' and myTestType = 'GT') THEN
	if isDebug > 0 then
		SET @infoText = CONCAT("invoking Gain Tracking calculations for frequency : ",myfreq);
		call debug(l_proc_id,@infoText,'I','I');
	end if;
	call pv_calc_Gain_Tracking(myTestId,myfreq,myTestDate);
-- Combination
  elseif ( myPolType = 'PV' and myTestType = 'CO') THEN
	if isDebug > 0 then
		SET @infoText = CONCAT("invoking Combination calculations for frequency : ",myfreq);
		call debug(l_proc_id,@infoText,'I','I');
	end if;
	call pv_calc_Combination(myTestId,myfreq,myTestDate);
 END IF;
  #Calculations - end
...
DELIMITER$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `pv_calc_Gain_Measurement`(
gmTestId INT,
...
IF done = 1 THEN
	if isDebug > 0 then
		...
		call debug(gm_proc_id,@infoText,'I','I');
	end if;
LEAVE the_loop;
END IF;
...
select MAX(Amplitude) into @A from pv_vdata where Prodserial_id = myserial and Frequency = gmFreq and datatype = 'M';

-- Received Amplitude of STD HORN -> B
select RecvdAmp into @B from pv_radata where Prodserial_id = myserial and Frequency = gmFreq;
...
-- AXIAL RATIO (dB) X
select pv_calc_AxialRatio(myserial,gmFreq,0) into @X;

-- AXIAL RATIO (UNITLESS) Y= Antilog (X / 20)
set Y = EXP(@X/20);

-- Gain correction Factor ,GF = 20 log (Y+1/v2*Y)
...
insert into pv_gmcalculated (Prodserial_id,Test_id ,Frequency,TestDate,
			...
			values
							(myserial, gmTestId, gmFreq, gmTestDate,
                            @A, @B, C, @D, Gv,
                            @X, Y, GF, CPGAIN);
...
DELIMITER$$
CREATE DEFINER=`root`@`localhost` FUNCTION `pv_calc_AxialRatio`(
cProdSerialId INT, freq decimal(40,20), degree INT
) RETURNS decimal(40,20)
BEGIN
...
-- HP
select Amplitude into @hp from pv_hdata where Prodserial_id = cProdSerialId and Frequency = freq
and Angle = degree and datatype = 'M';

-- VP
select Amplitude into @vp from pv_vdata where Prodserial_id = cProdSerialId and Frequency = freq
and Angle = degree and datatype = 'M';
